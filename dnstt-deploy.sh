...
SCRIPT_URL="https://raw.githubusercontent.com/begirkaro/dnstt-deploy/main/dnstt-deploy.sh"
...
PANEL_INSTALL_DIR="/opt/dnstt-panel"
# Panel source: same repo; override if you use a fork
...
show_configuration_info() {
    ...
    # Show panel info if SSH mode and panel installed
    if [ "$TUNNEL_MODE" = "ssh" ] && [ -f "${CONFIG_DIR}/panel.env" ]; then
        echo ""
...
get_user_input() {
    ...
    # Unattended: use pre-set values from env/args, only prompt for missing
    if [[ -z "$NS_SUBDOMAIN" && -n "$existing_domain" ]]; then
        NS_SUBDOMAIN="$existing_domain"
    fi
    if [[ -z "$MTU_VALUE" ]]; then
        MTU_VALUE="${existing_mtu:-1232}"
    fi
    if [[ -z "$TUNNEL_MODE" && -n "$existing_mode" ]]; then
        TUNNEL_MODE="$existing_mode"
    fi

    # Get nameserver subdomain (skip if already set)
    while true; do
        if [[ -n "$NS_SUBDOMAIN" ]]; then
            print_status "Using nameserver subdomain: $NS_SUBDOMAIN"
            break
        fi
    ...
    # Get MTU value (skip if already set)
    if [[ -z "$MTU_VALUE" ]]; then
        ...
        print_status "Using MTU: $MTU_VALUE"
    fi

    # Get tunnel mode (skip if already set)
    if [[ -n "$TUNNEL_MODE" && "$TUNNEL_MODE" = "socks" || "$TUNNEL_MODE" = "ssh" ]]; then
        print_status "Using tunnel mode: $TUNNEL_MODE"
    else
        while true; do
...
# Function to install SSH user management panel (SSH mode only)
install_dnstt_panel() {
    print_status "Installing DNSTT SSH user management panel..."

    # Install Python3 and venv if needed
    case $PKG_MANAGER in
        dnf|yum)
            $PKG_MANAGER install -y python3 python3-pip 2>/dev/null || true
            ;;
        apt)
            apt install -y python3 python3-venv python3-pip 2>/dev/null || true
            ;;
    esac

    if ! command -v python3 &>/dev/null; then
        print_error "Python3 is required for the panel. Install python3 and run again."
        return 1
    fi

    # Download panel from repo archive
    local tmp_archive="/tmp/dnstt-panel-archive.tar.gz"
    local tmp_extract="/tmp/dnstt-panel-extract"
    print_status "Downloading panel from repository..."
    if ! curl -sL "$PANEL_ARCHIVE_URL" -o "$tmp_archive"; then
        print_warning "Could not download panel archive. Skipping panel install."
        print_warning "You can add SSH users manually with: useradd -m -s /bin/bash <user> && passwd <user>"
        return 0
    fi
    ...
    if ! tar -xzf "$tmp_archive" -C "$tmp_extract"; then
        print_warning "Could not extract panel archive. Skipping panel install."
        rm -f "$tmp_archive"
        return 0
    fi

    # Find panel directory (could be dnstt-deploy-main/panel or similar)
    local panel_src
    panel_src=$(find "$tmp_extract" -type d -name "panel" 2>/dev/null | head -1)
    if [[ -z "$panel_src" || ! -f "$panel_src/app.py" ]]; then
        print_warning "Panel directory not found in archive. Skipping panel install."
        rm -rf "$tmp_extract" "$tmp_archive"
        return 0
    fi

    mkdir -p "$PANEL_INSTALL_DIR"
    cp -r "$panel_src"/* "$PANEL_INSTALL_DIR/"
    rm -rf "$tmp_extract" "$tmp_archive"

    # Create virtual environment and install dependencies
    print_status "Setting up panel Python environment..."
    if ! python3 -m venv "$PANEL_INSTALL_DIR/venv" 2>/dev/null; then
        print_warning "Could not create venv. Trying without venv..."
    else
        "$PANEL_INSTALL_DIR/venv/bin/pip" install -q -r "$PANEL_INSTALL_DIR/requirements.txt" 2>/dev/null || true
    fi

    # Run panel init (creates DB, random admin + port, panel.env)
    export DNSTT_CONFIG_DIR="$CONFIG_DIR"
    local panel_admin panel_pass panel_port
    panel_port="5847"
    ...
    if [[ -f "${CONFIG_DIR}/panel.db" ]]; then
        ...
        if [[ -z "$init_out" ]]; then
            init_out=$(python3 "$PANEL_INSTALL_DIR/init_panel.py" 2>/dev/null) || true
        fi

        panel_admin=$(echo "$init_out" | grep "^DNSTT_PANEL_ADMIN_USER=" | cut -d= -f2-)
        panel_pass=$(echo "$init_out" | grep "^DNSTT_PANEL_ADMIN_PASSWORD=" | cut -d= -f2-)
        panel_port=$(echo "$init_out" | grep "^DNSTT_PANEL_PORT=" | cut -d= -f2-)

        if [[ -z "$panel_port" ]]; then
            panel_port="5847"
            print_warning "Could not read panel port from init; using $panel_port"
        fi
...
EOF
    ...
    # Ensure panel.env exists and has PANEL_PORT for systemd
    if [[ -f "${CONFIG_DIR}/panel.env" ]]; then
        if ! grep -q "^PANEL_PORT=" "${CONFIG_DIR}/panel.env"; then
            echo "PANEL_PORT=$panel_port" >> "${CONFIG_DIR}/panel.env"
        fi
    fi

    # Systemd service for panel (runs as root for useradd/userdel/chpasswd)
    cat > "${SYSTEMD_DIR}/dnstt-panel.service" << EOF
[Unit]
Description=DNSTT SSH User Management Panel
After=network.target

[Service]
Type=simple
ExecStart=${PANEL_INSTALL_DIR}/venv/bin/python ${PANEL_INSTALL_DIR}/run_panel.py
WorkingDirectory=${PANEL_INSTALL_DIR}
Environment=DNSTT_CONFIG_DIR=${CONFIG_DIR}
Environment=DNSTT_PANEL_BASE=${PANEL_INSTALL_DIR}
...
EOF
    ...
    # Fallback if venv not used
    if [[ ! -x "$PANEL_INSTALL_DIR/venv/bin/python" ]]; then
        sed -i "s|${PANEL_INSTALL_DIR}/venv/bin/python|$(command -v python3)|g" "${SYSTEMD_DIR}/dnstt-panel.service"
    fi

    systemctl daemon-reload
    systemctl enable dnstt-panel
    systemctl start dnstt-panel 2>/dev/null || true

    # Firewall: allow panel port
    if command -v firewall-cmd &>/dev/null && systemctl is-active --quiet firewalld; then
...
main() {
    ...
    if [ "$0" != "$SCRIPT_INSTALL_PATH" ]; then
        ...
        # Unattended one-liner: parse args so one command does everything
        # Usage: bash <(curl -Ls URL) [SUBDOMAIN] [MTU] [MODE]
        # Example: bash <(curl -Ls URL) t.example.com 1232 ssh
        if [[ -n "$1" && "$1" != --* ]]; then
            NS_SUBDOMAIN="$1"
            MTU_VALUE="${MTU_VALUE:-1232}"
            TUNNEL_MODE="${TUNNEL_MODE:-ssh}"
        fi
        if [[ -n "$2" && "$2" =~ ^[0-9]+$ ]]; then
            MTU_VALUE="$2"
        fi
    ...
    # Install SSH user management panel (SSH mode only)
    if [ "$TUNNEL_MODE" = "ssh" ]; then
        install_dnstt_panel
    fi
